...
[阅读理解、价值与扩展汇总](notes/understanding.md) · [真实测试所需条件](real-wifi/README.md) · [实测失败线索](notes/live-input-diagnosis.md)
...
| 演示状态 | 本地已运行，未部署公网；AX211 网卡可读真实 RSSI，ESP32 CSI 硬件未连接 |
...
以下地址仅供启动服务后的本机使用，不是公开演示地址；历史原始记录不随仓库分发，其他电脑会显示该回放不可用。
...
依赖、源码锁定信息与运行说明均在 [web](web/README.md)。原版源码缓存和编译产物被忽略，不提交第三方仓库副本。若需要公网静态回放，再接入仓库 Pages 清单；实时 Rust 计算需要独立服务。

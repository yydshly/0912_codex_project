...
新增六能力网页呈现引导图与 `docs/web-guide.html` 导航页，并接入项目说明、首页和生成实验台。所有参考网页从 Markdown 构建，准备结果为 3 个应用页面 + 13 份参考页面。原有能力图、原理图及已发布应用保留。本批改动先完成本地核对，再由用户明确确认提交与发布；沿用统一构建与上线核验流程，下面另保留此前版本的线上记录。

本地构建与检查通过：16 个 HTML 页面、184 个站内引用；根索引与 OpenMAIC 文档共 15 份 Markdown、167 个相对链接有效。新图已目视核对，导航页本地 HTTP 200；未进行浏览器交互测试。

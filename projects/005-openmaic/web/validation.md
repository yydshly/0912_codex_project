# 网页验证记录

日期：2026-09-11。独立研究站点与欧姆定律互动案例；已安装并使用官方 renderer / DSL，没有运行完整上游应用或模型。

## 本次验证结果

- 13 组自动检查通过：原有课堂 7 组，生成实验台新增 6 组，涵盖六模块产物、错误输入、元素引用、课件文字边界、官方 DSL 与渲染、题目评分、项目约束及实验脚本计算和消息处理。
- 三页课件通过 `@openmaic/dsl 0.11.1` 的 `validateScene`；`@openmaic/renderer 0.1.6` 的 `SlideCanvas` 实际产生了包含三页公式内容的服务端 HTML。这验证组件可运行，不等于浏览器布局与高亮已实测。
- 完成 1 个总览、1 个互动课堂、1 个生成实验台、12 份参考文档的构建。站内链接、锚点、图片替代文本及六个能力入口检查通过。
- 课堂模块和依赖完整打包，检查没有未打包的运行时外部模块；法律注释和已安装生产依赖许可随产物保留。总览 JavaScript 语法检查通过。
- npm 安装完成后的审计结果为 0 个已报告漏洞；这是当时的包数据库结果，不代表未来或所有安全问题。
- 本地 HTTP 案例页面及主要资源已检查；2026-09-11 完成 GitHub Pages 发布，在线首页、案例、理解汇总、原理、比较及原理图均返回 HTTP 200。
- GitHub Pages 流程已增加 Node.js、锁定依赖安装、案例检查与构建；总项目继续保留 004、005 两个子站。

## 远端发布核验

- 生成实验台在 2026-09-11 随 [e55e9d6](https://github.com/yydshly/0912_codex_project/commit/e55e9d68c1b1804569d55d328a00415c354d602a) 发布，[工作流成功](https://github.com/yydshly/0912_codex_project/actions/runs/34572010462)，`generation.html`、入口脚本与样式文件均通过 HTTP 200 检查。后续修订沿用同一发布流程。
- 首次发布研究提交：[a2bfed1](https://github.com/yydshly/0912_codex_project/commit/a2bfed11c9fc94ca614224ed0f83d1e9304fe5fe)；[对应工作流](https://github.com/yydshly/0912_codex_project/actions/runs/34570237130)全部成功，包含案例检查、两项目构建和 Pages 部署。
- 在该提交的独立干净检出中安装锁定依赖、构建两站并汇总，对线上 **379 个文件**逐个检查 HTTP 与 SHA-256，全部一致；文本换行归一化，二进制按原字节比较。覆盖总入口、004 原有页面、005 的 14 个 HTML、图片及打包依赖。这是部署文件一致性检查，不是浏览器交互测试。
- 核验后补充了正式在线入口和本记录；后续更新继续由同一工作流发布，可在对应提交的干净检出中运行 `python scripts/check_pages.py` 复核。
- 在线入口：[能力总览](https://yydshly.github.io/0912_codex_project/005-openmaic/) · [理解汇总](https://yydshly.github.io/0912_codex_project/005-openmaic/docs/understanding.html) · [互动课堂](https://yydshly.github.io/0912_codex_project/005-openmaic/case.html)。

## 验证边界

- 新实验网页的 JavaScript 在 Node VM 的最小元素与消息替身中实际执行，验证初始计算、输入更新和父页面消息；这不是浏览器 iframe、布局或点击测试。
- 本次未请求浏览器自动化测试，未进行浏览器截图、点击或 DOM 检查；实际滑杆、下载、存储、语音、响应式和辅助技术兼容性尚未通过浏览器实测。
- 已实现键盘焦点、表单标签、状态提示、减少动态效果偏好和窄屏布局规则；这些是实现项，不冒充无障碍认证。
- 角色对话与讲解预先编写；参数实验和评分使用本案例的规则程序，不能代表上游模型生成内容的正确率。
- 未测量完整上游的生成耗时、费用、知识准确性、模型评分一致性或教学收益。

案例能力映射与版本来源见[案例说明](../notes/05-case-study.md)，重现命令见[网页运行说明](README.md)。
